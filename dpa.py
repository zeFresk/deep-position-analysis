...
def make_parser():
    ...
    parser.add_argument("--thresold", dest="thresold", action="store", type=int, default=25600, help="stop exploring further if the score (in centipawn) is above thresold.")
...
class Explorator(object):
    ...
    def explore(self, board, engine, pv, depth, nodes, msec = None, thresold = 25600,appending = True):
        """
    ...
    def _explore_rec(self, board, depth): # less parameters so less copy
        ...
        try:
            ...
            for (mo, score) in moves: # explore new moves
                ...
                new_board.push(mo)
                if not new_board.is_game_over(claim_draw=True) and not self.above_thresold(score): # If the game isn't drawn or won by a player we continue
    ...
    def above_thresold(self, score):
        """Returns wether a score is above thresold or not."""
        if score[1] == "M": # This a mate score !
            return 128.0 > self.thresold # A mate value is 128
        else:
            return abs(float(score)) > self.thresold
...
def main():
    ...
    for filename in args.fen_files:
        ...
        for (i, position_str) in enumerate(fens): #iterate through lines
            ...
            # Explore current fen
            exp = Explorator()
            tree = exp.explore(board, engine, args.pv, args.depth, args.nodes, msec, args.thresold, args.appending)
